...
public class GraphicRender {
    ...
    public void renderWaveform(Wave wave, float timeStep, String filename) {
        ...
        int width = (int) (nAmplitudes.length
                / wave.getWaveHeader().getSampleRate() / timeStep);
        int height = 500;
        int middle = height / 2;
        int magnifier = 1000;
        ...
        if (width > 0) {
            ...
            for (int i = 0; i < width; i++) {
                for (int j = scaledNegAmplitudes[i]; j < scaledPosAmplitudes[i]; j++) {
                    int y = height - j; // j from -ve to +ve, i.e. draw from top
                                        // to bottom
                    if (y < 0) {
                        y = 0;
                    } else if (y >= height) {
    ...
    public void renderSpectrogram(Spectrogram spectrogram, String filename) {
        renderSpectrogramData(spectrogram.getNormalizedSpectrogramData(),
                filename);
    }

    /**
     * Render a spectrogram data array
     * @param spectrogramData spectrogramData[time][frequency]=intensity, which
     *            time is the x-axis, frequency is the y-axis, intensity is the
     *            color darkness
     * @param filename output file
     * @see RGB graphic rendered
     */
    public void renderSpectrogramData(double[][] spectrogramData,
                                      String filename) {
        ...
        if (spectrogramData != null) {
            ...
            setBufferedImage(new BufferedImage(width, height,
                    BufferedImage.TYPE_INT_RGB));
            for (int i = 0; i < width; i++) {
                if (i == xMarker) {
                    for (int j = 0; j < height; j++) {
                        getBufferedImage().setRGB(i, j, 0xFF00); // green
                    }
                } else {
                    for (int j = 0; j < height; j++) {
                        int value;
                        ...
                        getBufferedImage().setRGB(i, height - 1 - j,
                                value << 16 | (255 - value) << 8);
                        // getBufferedImage().setRGB(i, height - 1 - j,
            ...
            try {
                int dotPos = filename.lastIndexOf(".");
                String extension = filename.substring(dotPos + 1);
                ImageIO.write(getBufferedImage(), extension, new File(filename));
            } catch (IOException e) {
                e.printStackTrace();
            }
        } else {
