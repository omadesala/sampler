/*
 * ID3Tag.java -- 解析MP3文件的ID3 v1/v2 tag. Copyright (C) 2010 This program is free
 ...
 * 如果你对其内置的图片感兴趣，可以用{@link #getPicture()}方法获取，图片的媒体类型为"image/png" 或
 * "image/jpeg"。
 * <p>
...
public class ID3Tag {
    ...
    public void parseID3V1(byte[] b, int off) {
        ...
        try {
            ...
            if (artist == null)
                artist = new String(buf, 30, i - 30, "gbk").replaceAll(
                        "[^\u4e00-\u9fa5]", "");
            if (artist.length() == 0)
                artist = null;
    ...
    private int getFrame(byte[] b, int off, int endPos) {
        ...
        if (len <= 0 || off + len > endPos || enc < 0
                || enc >= TEXT_ENCODING.length)
        ...
        try {
            switch (id.hashCode()) {
                ...
                case 2583398: // TRCK: 音轨
                    break;
                /*
                 * case 2614438: //USLT: 歌词 off += 4; //Languge: 4-byte len -=
                 * 4; //System.out.println(new String(b, off, len,
                 * TEXT_ENCODING[enc])); lyrics = new String(b, off, len,
                 * TEXT_ENCODING[enc]); break;
                 */
                case 2015625: // APIC
                    // MIMEtype: "image/png" or "image/jpeg"
                    for (id_part = off; b[id_part] != 0 && id_part < endPos; id_part++);
                    String MIMEtype = new String(b, off, id_part - off,
                            TEXT_ENCODING[enc]);
                    ...
                    if (apicData == null) {
                         ...
                         * FileOutputStream("apic."+ext); fos.write(b, off,
                         * len); fos.flush(); fos.close(); } catch (Exception e)
                         * {}
                         */
                    }
                    break;
