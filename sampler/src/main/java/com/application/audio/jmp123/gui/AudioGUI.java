/*
 * AudioGUI.java -- 音频输出及频谱显示 Copyright (C) 2010 This program is free software:
 ...
 * <http://jmp123.sf.net/>
 */
package com.application.audio.jmp123.gui;
...
import com.application.audio.jmp123.gui.album.AlbumFrame;
import com.application.audio.jmp123.gui.album.AlbumReceiver;
...
public class AudioGUI extends JComponent implements IAudio, AlbumReceiver {
    ...
    private int xpos; // display time-frq
    ...
    public AudioGUI(int sampleRate, Player p) {
        ...
        bkImage = new ImageIcon(getClass().getResource(
                "resources/image/bk1.jpg")).getImage();
        if (bkImage.getWidth(null) == -1)
            bkImage = null;

        spectrumBufferdImage = new BufferedImage(width, height,
                BufferedImage.TYPE_3BYTE_BGR);

        // this will be a line with different color
        timeSpectrumImage = new BufferedImage(1, height,
        ...
        setPlot(width);
        barBufferedImage = new BufferedImage(deltax - 1, height,
                BufferedImage.TYPE_3BYTE_BGR);
        render(0xe0e0ff, 0xff0000, 0xffff00, 0xb0b0ff);

        java.awt.Font f = spectrumGraphics.getFont();
        java.awt.Font myFont = new java.awt.Font(f.getName(),
    ...
    private void render(int rgbPeak, int rgbTop, int rgbMid, int rgbBot) {
        ...
        g2d.setPaint(new GradientPaint(0, 0, crTop, deltax - 1, height / 2,
                crMid));
        g2d.fillRect(0, 0, deltax - 1, height / 2);
        g2d.setPaint(new GradientPaint(0, height / 2, crMid, deltax - 1,
                height, crBot));
    ...
    private void drawHistogram(float[] amp) {
        ...
        for (; i != band; i++, x += deltax) {
            ...
            /*
             * maxAmp转换为用对数表示的"分贝数"y: y = (int) Math.sqrt(maxAmp); y /= FFT_N;
             * //幅值 y /= 8; //调整 if(y > 0) y = (int)(Math.log10(y) * 20 * 1.7);
        ...
        y = x / 60;
        String strMsg = String.format("[%02d:%02d] %02d:%02d", i, (int) (dura
                - i * 60 + 0.5), y, x - y * 60);
    ...
    public void close() {
        ...
        if (albumThread != null) {
            ...
            /*
             * try { albumThread.join(); } catch (InterruptedException e) { }
             */
        }
