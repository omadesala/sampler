...
public abstract class RandomRead {
     ...
     * @throws IOException 如果 pos 小于 0 或者发生 I/O 错误。
     */
     ...
     * 打开多路复用媒体流过程中查找音频流时对多路复用媒体流解包时已经获取到该媒体的播放时长。本方法应在调用
     * {@link #open(String, String)} 之后被调用。
     * <p>
     * 如果调用 {@link #open(String, String)}
     * 时指定的文件名是MPEG音频流媒体文件，调用本方法总是返回零。应该在完成帧头解码之后调用
