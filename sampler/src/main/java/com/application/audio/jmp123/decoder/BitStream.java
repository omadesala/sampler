/*
 * BitStream.java -- 读取位流 Copyright (C) 2010 This program is free software: you
 ...
 * 从输入的字节流获得位流。
 * <p>
 * 对于Layer3文件解码， BitStream 仅用于读取帧边信息，要读取Layer3文件的主数据，必须使用
 * {@link BitStreamMainData}。
 * <p>
...
public class BitStream {
     ...
     * @param extra 缓冲区尾部空出的字节数，防止哈夫曼解码时位流有错误导致缓冲区溢出，尾部空出512字节(part2_3_length长度，
     *            2^12位)。
     */
    ...
    public int append(byte[] b, int off, int len) {
        if (len + endPos > maxOff) {
            ...
            System.arraycopy(bitReservoir, bytePos, bitReservoir, 0, endPos
                    - bytePos);
            endPos -= bytePos;
            bitPos = bytePos = 0;
        }
