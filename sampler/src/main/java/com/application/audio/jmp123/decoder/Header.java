/*
 * Header.java --MPEG-1/2/2.5 Audio Layer I/II/III 帧同步和帧头信息解码 Copyright (C) 2010
 ...
 * along with this program. If not, see <http://www.gnu.org/licenses/>. If you
 * would like to negotiate alternate licensing terms, you may do so by
 * contacting the author: <http://jmp123.sourceforge.net/>.
 */

package com.application.audio.jmp123.decoder;
...
public final class Header {
    ...
    // public static final int MAX_FRAMESIZE = 1732; //MPEG 1.0/2.0/2.5, Layer
    // 1/2/3
    ...
    /*
     * verID: 2-bit '00' MPEG-2.5 (unofficial extension of MPEG 2); '01'
     * reserved; '10' MPEG-2 (ISO/IEC 13818-3); '11' MPEG-1 (ISO/IEC 11172-3).
     */
    private int verID;
    ...
    /*
     * sampling_frequency: 2-bit '00' 44.1kHz '01' 48kHz '10' 32kHz '11'
     * reserved
     */
    ...
    /*
     * mode: 2-bit '00' Stereo; '01' Joint Stereo (Stereo); '10' Dual channel
     * (Two mono channels); '11' Single channel (Mono).
     */
    private int mode;
    ...
    private boolean available(int h, int curmask) {
        return (h & curmask) == curmask && ((h >> 19) & 3) != 1 // version ID:
                                                                // '01' -
                                                                // reserved
     ...
     * 本方法执行的操作：
     * <ol>
     * <li>查找源数据缓冲区 b 内帧同步字（syncword）。</li>
     * <li>如果查找到帧同步字段：</li>
     * <ol type="I">
     * <li>解析帧头4字节。</li>
     * <li>如果当前是第一帧，解码VBR信息。</li>
     * </ol>
     * <li>返回。</li>
     * <ul>
     * <li>若返回<b>true</b>表示查找到帧同步字段， 接下来调用 {@link #getVersion()}、
     * {@link #getFrameSize()} 等方法能够返回正确的值。</li>
     * <li>若未查找到帧同步字段，返回<b>false</b>。</li>
     * </ul>
     * </ol>
     * @param b 源数据缓冲区。
    ...
    private void parseVBR(byte[] b, int off) {
        ...
        float peak = Float.intBitsToFloat(byte2int(b, off)); // Peak signal
                                                             // amplitude
        off += 4;
        int radio = byte2short(b, off); // Radio Replay Gain
        /*
         * radio: bits 0h-2h: NAME of Gain adjustment: 000 = not set 001 = radio
         * 010 = audiophile bits 3h-5h: ORIGINATOR of Gain adjustment: 000 = not
         * set 001 = set by artist 010 = set by user 011 = set by my model 100 =
         * set by simple RMS average bit 6h: Sign bit bits 7h-Fh: ABSOLUTE GAIN
         * ADJUSTMENT. storing 10x the adjustment (to give the extra decimal
         * place).
         */
        off += 2;
         ...
         * //000?0000: LAME uses "--nspsytune" ? boolean nsp = ((enc_flag & 0x1)
         * == 0) ? false : true; //00?00000: LAME uses "--nssafejoint" ? boolean
         * nsj = ((enc_flag & 0x2) == 0) ? false : true; //0?000000: This track
         * is --nogap continued in a next track ? //is true for all but the last
         * track in a --nogap album boolean nogap_next = ((enc_flag & 0x4) == 0)
         * ? false : true; //?0000000: This track is the --nogap continuation of
         * an earlier one ? //is true for all but the first track in a --nogap
         * album boolean nogap_cont = ((enc_flag & 0x8) == 0) ? false : true;
         */
        off++;
        ...
        // "Peak signal amplitude", "Radio Replay Gain",
        ...
        if (mp3_gain != 0)
            System.out.println("    MP3 Gain: " + mp3_gain + " [psa=" + peak
                    + ",rrg=" + radio + ",arg=" + phile + "]");
    ...
    public void printHeaderInfo() {
        ...
        String strDuration = String.format("%02d:%02d", m, (int) (duration - m
                * 60 + 0.5));
        if (strBitRate == null)
            strBitRate = String.format("%dK",
                    bitrate[lsf][layer - 1][bitrate_index]);
    ...
    public void printProgress() {
        ...
        System.out.printf("\r#%-5d [%-41s] %02d:%05.2f ", framecounter,
                progress, m, s);
    }
